...
9. MEXC Futures проверяется отдельно по живому USDT-фьючерсу: сначала точный `BASE_USDT`, затем безопасный расширенный контракт MEXC вроде `BASE..._USDT` для тикеров от 4 символов.
...
- `/mexc BTC` - диагностика MEXC Futures по `BTC_USDT` и безопасному расширенному USDT-контракту, если MEXC использует длинное имя.
...
- `Да` ставится, если `ccxt` или MEXC contract API подтвердили активный USDT-фьючерс: точный `BASE_USDT` или безопасный расширенный контракт, где база начинается с `BASE` и сам запрос не короче 4 символов.
